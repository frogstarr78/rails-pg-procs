    ...
    with_proc(:update_trade_materials_statuses_logf, [], :return => :trigger) {
      with_trigger(:test_table, [:insert, :update], :before => true, :name => :update_trade_materials_statuses_logt, :function => :update_trade_materials_statuses_logf) {
        assert_no_exception(NoMethodError) do 
          ...
          stream.rewind
          assert stream.read.include?("add_trigger \"test_table\", [:insert, :update], :before => true, :name => :update_trade_materials_statuses_logt, :function => :update_trade_materials_statuses_logf")

          stream = StringIO.new
          dumper.send(:procedures, stream)
          stream.rewind
          received_sql = stream.string
          assert_match "  create_proc \"update_trade_materials_statuses_logf\", [], :return => :trigger, :resource => ['', '\nBEGIN\n-- do something --\nEND;\n\n'], :lang => 'plpgsql'\n".to_regex, received_sql
...
END;" }
    ...
    @connection.drop_table "somewhere.table_in_a_specific_schema" if @connection.tables.include?('table_in_a_specific_schema')
    ...
    ['[:name, "character varying"]', '[:address, "character varying(20)"]', '[:zip, "character varying(5)"]', '[:phone, "numeric(10,0)"]'].each do |fragment|
      assert(received_sql[%r|create_type "qualitysmith_user"(.*)|, 1].include?(fragment))
    end
    ...
    assert_match(%q|create_table "table_in_a_specific_schema"|.to_regex, received_sql)
    assert_match("sql_proc_with_table_reference\", [:int4], :return => :int4, :resource => ['', '\nSELECT id FROM a_table_defined_after_the_stored_proc WHERE id = $1;\n'], :lang => 'sql'".to_regex, received_sql.split("\n")[-9..-2].join("\n"))
