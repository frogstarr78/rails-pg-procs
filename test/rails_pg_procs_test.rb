    ...
    with_proc(:update_trade_materials_statuses_logf, [], :return => :trigger) {
      with_trigger(:test_table, [:insert, :update], :before => true, :name => :update_trade_materials_statuses_logt, :function => :update_trade_materials_statuses_logf) {
        assert_no_exception(NoMethodError) do 
          ...
          stream.rewind
          assert_match 'add_trigger "test_table", [:insert, :update], :before => true, :name => :update_trade_materials_statuses_logt, :function => :update_trade_materials_statuses_logf'.to_regex, stream.read
...
END;" }
    ...
    assert_match('create_schema(:somewhere_else, "postgres")'.to_regex, received_sql)
    ...
    assert_match('create_table("somewhere.schema_in_a_specific_table"'.to_regex, received_sql)
  ...
  def test_schema_dumper_type
    ...
    assert_no_exception(NoMethodError) do 
      ...
      stream.rewind
      received = stream.read
       assert_match 'create_type "qualitysmith_user", [:name, "character varying(10)"], [:zip, "numeric(5,0)"], [:is_customer, :boolean]'.to_regex, received.chomp
    end
    @connection.drop_type(:qualitysmith_user)
  end
    ...
    with_proc(:insert_after_test_table_trigger, [], :return => :trigger) {
      with_trigger(:test_table, [:insert], :row => true) {
        assert_no_exception(NoMethodError) do 
          ...
          dumper.send(:triggers, :test_table, stream)
          stream.rewind
          assert_match 'add_trigger "test_table", [:insert], :row => true'.to_regex, stream.read
